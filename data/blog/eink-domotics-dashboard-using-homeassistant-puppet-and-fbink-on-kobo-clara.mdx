...
An always-on dashboard in the house has been on my wishlist for quite a while. Somehow, I never really went for it before. Recently I saw a live PV / Grid consume / Grid delivery panel at a friend's house, and with the recent [pv solar wifi stick mod](./growatt-wifi-stick-modification-to-make-uart-work-with-esp-home.mdx) in place, it was about time for a first prototype of this solution. 
...
The lazy plan would be to point the Kobo's built-in browser at a home assistant dashboard in a sort of kiosk mode and call it a day. Unfortunately, that does not work as the experimental browser can't run the home assistant dashboard without javascript.
...
For this rendering I use balloob's [puppet](https://github.com/balloob/home-assistant-addons) add-on, a headless Chrome that screenshots any home assistant view. Home Assistant was already running in a docker-compose, so we only had to add the extra container. Requesting a URL and getting a grayscale PNG back:
