...
- A `Certificate` for our (sub)domain, managed by certmanager
...
We start off by creating the namespace. For this example, let's imagine we are exposing a homeassistant server running on a separate server within our network
...
For some reason, I was stuck a long time on configuring this endpoint, as I was sent down a rabbit hole of the EndpointSlices I couldn't find good examples for. The thing that's weird here is that there are no references from and to the service definition, other than the name. So if you configure an Endpoints with the same name as the Service, the service will use the endpoint that you specified manually.
```
kind: Endpoints
apiVersion: v1
metadata:
...
Similar to before, we create a Traefik `IngressRoute`, which is the custom Traefik version of a Kubernetes `Ingress`. We specify the port and service, and specify the secret that contains the ssl certificate, as specified in the certificate from earlier in this post.
```
