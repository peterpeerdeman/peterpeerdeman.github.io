...
Instead of using the spiderable package to fully render the application including all the logic and data in the blaze template we decided to serve alternative snippets for our SEO heavy content whenever google, facebook or another crawler hits the application server with a specific user agent.
...
The following piece of meteor backend code shows how to create a router that is triggered when the botagent or google's "unescaped fragment" query param is detected.
