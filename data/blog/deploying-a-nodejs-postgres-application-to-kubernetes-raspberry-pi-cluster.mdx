...
Now the Raspberry Pi Kubernetes cluster is [up and running](../blog/kubernetes-cluster-build-with-raspberry-pi-and-poe-hats) and is carefully being [monitored](../blog/deploying-monitoring-tig-stack-telegraf-influxdb-grafana-on-raspberry-pi-kubernetes-cluster), it is about time to start deploying some actual workloads to the cluster.
...
Now the application is properly packaged up, we can start working on the actual kubernetes files. I first followed an [excellent guide by Daniel Olaogun](https://loft.sh/blog/adding-services-and-dependencies-in-kubernetes-tutorial/) to set up the deployment and services for the app. I made a couple of adjustments to support postgres instead of mysql, and made it work with a stateful set instead of a deployment for the database. 
...
You can see our deployment has two dependencies on the postgres database: the definition references a secret containing the postgres database url containing username, password, host, port and database name. The specification of the host even reveals a second dependency on a service called `postgres-service`, but we'll get to that later.  Before we can apply the definition to our cluster, we first create the secret in a fresh new namespace:
...
As you can see, these two definitions expose both services. The `type` is the most interesting part here. The postgres one uses a [ClusterIP](https://kubernetes.io/docs/concepts/services-networking/cluster-ip-allocation/), which means it will get an ip address automatically within the cluster range. The type `loadBalancer` creates an externally accessible IP that we can route external traffic to, but we'll probably get into that next time.
...
3. [Longhorn for persistent, replicated storage on raspberry pi kubernetes cluster](../blog/installing-longhorn-for-persistent-replicated-storage-on-raspberry-pi-kubernetes-cluster)
