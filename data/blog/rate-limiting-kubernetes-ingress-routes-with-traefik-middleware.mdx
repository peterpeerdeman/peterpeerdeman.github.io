...
There are very elaborate solutions to doing extensive hardening on kubernetes services such as [crowdsec](https://github.com/crowdsecurity/crowdsec) which is similar to solutions such as fail2ban we had back in the apache2 days. But if you really quickly just want to put some ratelimiting up on a service, you can far more easily accomplish this with Traefik Middlewares.

To for instance ratelimit an IngressRoute, we add a special type `Middleware` entity, describe its spec variables and add it to the existing ingressroute directly under the route configuration. Apply the file and the rate-limited endpoint is good to go!
