...
The blog series about collecting timeseries, making predictions and making these predictions configurable is now coming to an epic conclusion where we tie all the previous steps together and add a workflow engine to create a proper Machine Learning Operations (MLOps) pipeline that runs every x amount of time.
...
- the `successfulJobsHistoryLimit` and `failedJobsHistoryLimit` have saved me from completely flooding the pod view in [k9s](./using-the-k9s-cli-tool-to-get-a-clear-overview-and-quick-acces-to-anything-on-your-cluster)
