...
The hardware part of the build was a lot of fun to set up. Instead of having the pi's lie around separately I've opted to go for a DIN breaker box panel, similar to the one routing the power in our home's breaker box. The Raspberry pi's with PoE hats fit snugly into the cases, which can then be clicked onto the DIN rail inside the Eaton Holec Panel.
...
8. Running the first Kubernetes commands on the **controller** node:
