...
images: ['/assets/images/2024-11-09-longhorn-eviction.png']
draft: false
---
...
If I would kill the pod, the CPU would go back down, but of course kubernetes would detect this, recreate the instance manager pod and the CPU problem would re-appear. Inspecting the logs of the instance manager, I found no noticeable clues other than an update watch running: 
...
Because it was only the control plane having these issues, I continued to find out if I could decrease the activity on the control plane through the longhorn ui. 
