...
After struggling for a couple of hours I found out why my Restangular POST call to our Symfony2 backend was not working. Symfony2 expects resources to be wrapped with an entity object while sending a POST or PUT.
...
I solved this dilemma by making the backend return a non wrapped resource so the id is parsed correctly and using a requestInterceptor to wrap the outgoing POST / PUT request with the object name:
