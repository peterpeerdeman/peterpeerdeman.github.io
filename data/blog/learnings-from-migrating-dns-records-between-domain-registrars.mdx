...
I try to avoid it at any time, but sometimes migrating DNS records from one registrar to another is just unavoidable. In this case it would have been great if the registrar would have allowed internal moving between accounts, so the domains could be kept alive while moving between different accounts for bookkeeping reasons.
...
Many domain hosts have different standards for how to store and parse records.
