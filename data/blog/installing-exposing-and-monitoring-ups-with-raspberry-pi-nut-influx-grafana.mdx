...
images: ['/assets/images/2021-10-22-connecting-and-monitoring-ups-with-raspberry-pi.png']
draft: false
---

To protect my Raspberry Pi server against sudden power outage and power fluctuations I've decided to install an "Uninterruptible Power Supply", or "UPS". I got the [APC Back-UPS 2200VA, 230V, AVR, 4 Schuko outlets](https://www.apc.com/shop/ug/en/products/APC-Back-UPS-2200VA-230V-AVR-4-Schuko-outlets/P-BX2200MI-GR) that gives my raspberry pi a window of about 1 hour to properly shut down without damaging the hardware or losing data.
...
We now have a server application running on port 3493 that exposes the UPS statistics detected through the USB cable coming from the UPS. Back in the day, there was no REST, no JSON and not even XML or SOAP. You can only test the API with [netcat](https://en.wikipedia.org/wiki/Netcat), connecting to the port directly and sending ASCII commands such as `LIST UPS` and `LIST VAR <upsname>` to retrieve the data:
